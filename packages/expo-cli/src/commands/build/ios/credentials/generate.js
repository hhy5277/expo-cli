...
async function generate(appleCtx, credentialsToGenerate, existingCredentials) {
  ...
  for (const id of credentialsToGenerate) {
    ...
    try {
      const generated = await _create(appleCtx, id, newCredentials);
...
async function _create(appleCtx, _type, otherCredentials) {
  const manager = apple.createManagers(appleCtx)[_type];
  return await manager.create(otherCredentials);
}
...
function determineMissingCredentials(existingCredentials = {}) {
  ...
  //   return toGenerate.reduce((acc, i) => {
  //     acc.push(i);
  //     const dependency = CREDENTIALS_DEPENDENCIES[i];
  //     if (dependency && !_.includes(toGenerate, dependency)) {
  //       acc.push(dependency);
  //     }
  //     return acc;
  //   }, []);
